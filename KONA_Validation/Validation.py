...
import io
import numpy as np
from PyQt5.QtWidgets import (QApplication, QFileDialog, QTextEdit, QComboBox, QVBoxLayout, QWidget,
                             QPlainTextEdit, QPushButton, QDesktopWidget, QGridLayout, QLabel, QLineEdit, QRadioButton)
...
def bioProject_Validation(targetSheet, sheetName, errbox):
    ...
    if Common.notMatchedFieldName(targetSheet, 'A18', 'Release date selection'):
        ...
        '''
        errbox.insertPlainText(
            '[ERROR] [' + str(sheetName) + '] "Release date selection 필드의 위치가 템플릿 양식과 일치하지 않습니다." (18 row)\n')
        flag += 1
    else:
        if str(targetSheet[str('E') + str(18)].value) == "Release on specified date":
            if not str(targetSheet[str('E') + str(19)].value).count('-') == 2:
                errbox.insertPlainText(
                    '[ERROR] [' + str(sheetName) + '] "Release on specified date" 를 선택한 경우 반드시 공개날짜를 입력해야합니다.(19 row,입력형식:YYYY-MM-DD)\n')
                flag = 1
    ...
    if Common.notMatchedFieldName(targetSheet, 'A21', 'Government department (국문)'):
        errbox.insertPlainText(
            '[ERROR] [' + str(sheetName) + '] "Government department 필드의 위치가 템플릿 양식과 일치하지 않습니다." (21 row)\n')
        flag += 1
    else:
        if str(object=targetSheet[str('E') + str(21)].value) not in ['과기정통부', '해양수산부', '보건복지부', '농림축산부', '산업부', '농진청', '산림청', ]:
            errbox.insertPlainText(
                '[ERROR] [' + str(sheetName) + '] "Government department" 선택 입력 값이 잘못되었습니다. (21 row)\n')
            flag += 1

    # Project type Check
    if Common.notMatchedFieldName(targetSheet, 'A26', 'Project type'):
        errbox.insertPlainText(
            '[ERROR] [' + str(sheetName) + '] "Project type 필드의 위치가 템플릿 양식과 일치하지 않습니다." (26 row)\n')
        flag += 1
    else:
...
def bioSample_Validation(targetSheet, compareSheet, sheetName, errbox):
    ...
    if str(targetSheet[str('E') + str(20)].value) == "Release on specified date":
        if not str(targetSheet[str('E') + str(21)].value).count('-') == 2:
            errbox.insertPlainText(
                '[ERROR] [' + str(sheetName) + '] "Release on specified date" 를 선택한 경우 반드시 공개날짜를 입력해야합니다.(21 row,입력형식:YYYY-MM-DD\n')
            flag += 1
    ...
    if Common.notMatchedFieldName(targetSheet, 'A17', 'Project accession '):
        errbox.insertPlainText(
            '[ERROR] [' + str(sheetName) + '] "Project accession 필드의 위치가 템플릿 양식과 일치하지 않습니다." (17 row)\n')
        flag += 1
    else:
        if Common.matchedNA(targetSheet[str('E') + str(17)].value):
            errbox.insertPlainText(
                '[ERROR] [' + str(sheetName) + '] "Project accession" 값을 입력해야합니다.(17 row)\n')
            flag += 1

    if Common.notMatchedFieldName(targetSheet, 'A27', 'Sample type'):
        errbox.insertPlainText(
            '[ERROR] [' + str(sheetName) + '] "Sampletype type" 필드의 위치가 템플릿 양식과 일치하지 않습니다. (27 row)\n')
        flag += 1
    else:
        '''
...
def sampleType_Validation(targetSheet, sheetName, errbox):
    ...
    if len(duplicationCheckArr) != len(duplicationCheckSet):
        errbox.insertPlainText(
            '[ERROR] [' + sheetName + '] "데이터 리스트 중에 이름을 제외한 모든 값이 중복되는 데이터 쌍이 있습니다."\n')
        flag += 1
...
def Experiment_Validation(targetSheet, sheetName, errbox):
    ...
    if not len(duplicationCheckArr) == len(duplicationCheckSet):
        errbox.insertPlainText(
            '[ERROR] [' + str(sheetName) + '] E~U 열이 모두 중복되는 데이터가 있습니다.\n')
        flag += 1
    ...
    if not len(forFilePath) == len(forFilePathSet):
        errbox.insertPlainText(
            '[ERROR] [' + str(sheetName) + '] "File path + File name"이 중복되는 데이터가 있습니다.\n')
        flag += 1
